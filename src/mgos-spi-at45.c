...
void ati_spi_flash_destroy(struct ati_spi_flash* _dev){
    if ( NULL != _dev ){
        ...
        free(_dev );
        _dev = NULL;
        LOG (LL_INFO, ("UNS-PA SPI Flash has being destroyed (CS/RESET/ISOL: %d/%d/%d)", cs, res, is) );
    }
    else
...
void ati_spi_flash_wait_idle_status (struct ati_spi_flash* _dev){
    ...
    LOG(LL_INFO, ("SPI FLASH wait for idle status: %d ms", (MAX_WAIT - cntr)));
}
...
uint8_t ati_spi_flash_write_from_buffer_to_page (struct ati_spi_flash* _dev, uint16_t _page_number ){
    ...
    LOG(LL_INFO, ("SPI FLASH move data from SRAM buffer to MainMemory (cmd: %02X/%02X/%02X/%02X, page size: %d bytes )",
                    tx_data[0],
                    tx_data[1],
                    tx_data[2],
                    tx_data[3],
...
uint8_t ati_spi_flash_read_from_page_and_offset(struct ati_spi_flash* _dev, uint8_t* _data, size_t _data_len, uint16_t _page_number, uint16_t _offset){
    ...
    LOG(LL_INFO, ("SPI FLASH read data from MainMemory (cmd: %02X/%02X/%02X/%02X, page size: %d bytes )",
            tx_data[0],
            tx_data[1],
            tx_data[2],
            tx_data[3],
